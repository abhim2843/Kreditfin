...
const stats = [
  ...
  { value: "20,000+", label: "Account Settled" },
  { value: "20L+", label: "Customers Counselled" },
  { value: "4.7/5", label: "3,000+ Reviews" },
];

const whyCards = [
  { title: "Lower Interest Rate", desc: "We help identify consolidation options that may reduce your overall interest burden." },
  { title: "Simplified Repayment", desc: "Helping customers reduce EMI burden and achieve lasting financial stability." },
  { title: "Transparent Guidance", desc: "Get unbiased recommendations tailored to your financial situation." },
  { title: "Dedicated Support", desc: "Expert assistance throughout your consolidation journey." },
];

const whyIcons = [
  // lower interest (percent down)
  ...
  // simplified repayment (merge)
  <svg key="1" width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M4 6h7M4 12h5M4 18h7M14 9l4 3-4 3M9 12h9" stroke="#4caf50" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" /></svg>,
  // transparent (eye/shield)
  <svg key="2" width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M12 3l7 3v5c0 4.5-3 7.5-7 9-4-1.5-7-4.5-7-9V6l7-3zM9 12l2 2 4-4" stroke="#4caf50" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" /></svg>,
  // support (headset)
...
const beforeRows = ["Multiple EMIs", "Multiple Due Dates", "Complex Tracking", "Higher Financial Stress"];
const afterRows = ["Single EMI", "Unified Due Dates", "Simple Tracking", "Better Financial Control"];

// one icon per row (shared between Before/After, recoloured per side)
const rowIconPaths = [
  "M12 3l9 5-9 5-9-5 9-5zM3 12l9 5 9-5M3 16.5l9 5 9-5", // stack / EMIs
  "M7 3v3M17 3v3M4 8h16M5 6h14a1 1 0 011 1v12a1 1 0 01-1 1H5a1 1 0 01-1-1V7a1 1 0 011-1z", // calendar / due dates
  "M4 19V5M4 19h16M8 16v-5M12 16V8M16 16v-8", // chart / tracking
  "M20.8 5.6a5 5 0 00-7.1 0L12 7.3l-1.7-1.7a5 5 0 10-7.1 7.1L12 21.5l8.8-8.8a5 5 0 000-7.1z", // heart / stress
];
...
const tips = [
  ...
  "Verify Property Documents Beforehand",
  "Maintain Stable Employment History",
];

const quickSteps = [
  { title: "Share Your Loan Details", desc: "Tell us about your current EMIs, lenders, and outstanding balances in just a few minutes." },
  { title: "We Find The Best Option", desc: "Our experts evaluate suitable consolidation opportunities." },
  { title: "Enjoy One Simplified EMI", desc: "Manage repayments through a single consolidated EMI." },
];

const faqs = [
  {
    question: "What is Multiple EMI Consolidation?",
    answer:
      "Multiple EMI Consolidation combines several loan EMIs and credit card repayments into one single, manageable EMI — often at a lower interest rate.",
  },
  {
    question: "Can I consolidate credit card dues and loans together?",
    answer:
      "Yes. You can combine credit card outstanding, personal loans, app loans and other unsecured debts into a single consolidated repayment.",
  },
  {
    question: "Will consolidation reduce my EMI?",
    answer:
      "In most cases, yes. By securing a lower interest rate and a longer tenure, customers typically reduce their monthly EMI by 20–40%.",
  },
  {
    question: "How long does the process take?",
    answer:
      "From application to disbursement, consolidation usually takes 7–14 business days. Our team works to expedite approvals for a smooth transition.",
  },
  {
    question: "Will consolidation affect my credit score?",
    answer:
      "Checking eligibility does not impact your score. Once consolidated, timely single-EMI payments often help improve your credit profile over time.",
  },
];
...
export default function MultipleEmiConsolidationPage() {
  return (
    <main className="min-h-screen bg-white">
      <Navbar />

      {/* Hero */}
      <section className="w-full relative overflow-hidden" style={{ background: "linear-gradient(120deg, #eef7ee 0%, #f0f9ff 100%)" }}>
        {/* Right illustration — bleeds to the viewport right edge (desktop) */}
        <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 w-[54%] max-w-[860px] h-full max-h-[470px]">
          <Image
            src="/assets/emi-hero-illustration.png"
            alt="Multiple EMIs into one"
            fill
            priority
            sizes="55vw"
        ...
        <div className="relative max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-[100px] py-12 lg:py-0 flex flex-col lg:flex-row items-center lg:min-h-[480px]">
          <div className="flex flex-col gap-6 w-full lg:max-w-[600px] z-10">
            <h1 className="text-[30px] sm:text-[44px] lg:text-[52px] font-extrabold leading-[38px] sm:leading-[54px] lg:leading-[60px]">
              <span className="text-[#1a1f2e]">Multiple EMI Consolidation</span><br />
              <span className="text-[#4caf50]">Into One EMI</span>
            </h1>
          ...
          <div className="lg:hidden w-full relative aspect-[973/585] mt-6">
            <Image src="/assets/emi-hero-illustration.png" alt="Multiple EMIs into one" fill priority sizes="100vw" className="object-contain" />
          </div>
        </div>
      </section>
            ...
            {stats.map((s, i) => (
              <div key={i} className="relative lg:flex-1 flex flex-col items-center text-center gap-2 px-2">
                <div className="w-12 h-12 rounded-[8px] bg-[rgba(76,175,80,0.1)] flex items-center justify-center">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M3 17l6-6 4 4 7-7M21 8v5M21 8h-5" stroke="#4caf50" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" /></svg>
                </div>
      ...
      <section className="w-full bg-[#f9fafc] py-12 sm:py-16">
        <div className="max-w-[1240px] mx-auto px-4 sm:px-8 lg:px-[100px] text-center">
          <h2 className="text-[26px] sm:text-[32px] font-bold text-[#1a1f2e] leading-[34px] sm:leading-[40px]">What is Multiple EMI Consolidation?</h2>
          <p className="text-[15px] sm:text-[18px] text-[rgba(26,31,46,0.7)] mt-4 leading-[26px] sm:leading-[30px]">
            Combine multiple loan EMIs and credit card repayments into a single manageable EMI. Simplify your finances, reduce repayment stress, and improve monthly cash flow. Our experts help you explore consolidation options tailored to your financial situation.
          </p>
        </div>
      </section>
      ...
      <section className="w-full bg-white py-12 sm:py-16">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-[100px]">
          <SectionHead title="See How Multiple EMI Consolidation Works" sub="Turn multiple repayments into one manageable EMI and regain control of your finances." />
      ...
      <section className="w-full bg-[#f9fafc] py-12 sm:py-16">
        <div className="max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-[100px]">
          <SectionHead title="Why Choose Kreditfin for EMI Consolidation" sub="We combine creative thinking with data-driven strategies to deliver results that matter for your financial journey." />
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-[100px]">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            <div className="bg-white rounded-[16px] p-6 sm:p-8 shadow-[0_4px_40px_rgba(0,0,0,0.06)] flex flex-col gap-5">
              <LeadForm subject="New KreditFin enquiry (Multiple Emi Consolidation)" />
            </div>
          </div>
        </div>
      </section>
